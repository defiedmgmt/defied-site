...
export async function releaseTabToPool(sheets, spreadsheetId, tabTitle, tabs) {
  ...
  // Clear only the site-owned columns — same set clearSongRow touches (A,
  // B, D, M, N) — never the formula columns (C,E,F,G,H,I,J,K,L). Those are
  // a permanent fixture of every row from row 5 through the Total row,
  // present whether or not the row holds a real song (confirmed against an
  // untouched WRITER tab); clearing them blanks the row's math entirely and
  // there's no way to regenerate the exact formulas from here. Also never
  // touches the Total row itself or anything below it.
