...
export async function releaseTabToPool(sheets, spreadsheetId, tabTitle, tabs) {
  ...
  // clear only the data rows above the Total row — never the Total row's
  // own SUM formulas or anything below it (see the note by totalRowIndex).
  const { totalRow } = await totalRowIndex(sheets, spreadsheetId, t.title);
  const lastRow = totalRow === null ? 64 : totalRow - 1;
  if (lastRow >= DATA_START_ROW) {
    await sheets.spreadsheets.values.clear({ spreadsheetId, range: `'${qt(t.title)}'!A${DATA_START_ROW}:N${lastRow}` });
  }
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId,
