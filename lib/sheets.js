...
// Reads just the Luminate ID cell of an existing row — used to preserve it
// when a non-staff push must never be allowed to overwrite it (a client
// session never receives the real Luminate ID, so its own copy is always
// blank; see sheets-push.js).
export async function readLuminateIdAt(sheets, spreadsheetId, tabTitle, row) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: `'${qt(tabTitle)}'!M${row}` });
...
// --- A&R: OUTREACH + ADVANCE CALCULATOR + ASSUMPTIONS ------------------
// Both tabs are deliberately walled off from MASTER/roster totals — never
// read by anything the client-facing dashboard-data or sheets-pull/push
// routes touch. Read-only from the site's side; the outreach tab itself
// (now reworked to hold up to 5 song-level Gross Streams/Writer % pairs
// per prospect row, columns R:AA, summing into the existing Gross
// Streams/Net Streams columns) is still edited by hand in the sheet.

const OUTREACH_TAB = "OUTREACH";
const ADVANCE_TAB = "ADVANCE CALCULATOR";
const OUTREACH_FIRST_ROW = 5;

export function tabLink(spreadsheetId, sheetId) {
  return `https://docs.google.com/spreadsheets/d/${spreadsheetId}/edit#gid=${sheetId}`;
}

// The four global levers every dollar formula on both tabs reads from.
// B7 is currency-formatted ("$0.00250" -> parseNum already gives 0.0025);
// B8/B9/B11 are percent-formatted ("25.0%" -> parseNum strips the "%" and
// gives 25, not 0.25) so those three need an extra /100 to land on the
// same 0-1 fraction scale the formulas actually multiply by.
export async function readAssumptions(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "'ASSUMPTIONS'!B7:B11" });
  const rows = res.data.values || [];
  return {
    blendedRate: parseNum(rows[0]?.[0]),        // B7 — $/stream
    decayRate: parseNum(rows[1]?.[0]) / 100,    // B8 — YoY decay
    runRate: parseNum(rows[2]?.[0]) / 100,      // B9 — forward run-rate
    adminFeeRate: parseNum(rows[4]?.[0]) / 100, // B11 — admin/management fee
  };
}

// The two manual inputs the ADVANCE CALCULATOR's own row formulas read
// from ($B$4 recoup target) — starting budget lives at $A$2 but nothing in
// the per-prospect formula chain uses it, only the tab's own top-line
// budget tracker, which this deliberately doesn't replicate site-side.
export async function readAdvanceCalcSettings(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: `'${ADVANCE_TAB}'!B4` });
  return { recoupTargetMonths: parseNum(res.data.values?.[0]?.[0]) || 12 };
}

// The tab's own top-line budget tracker: A2 starting budget (manual),
// E2 committed (=SUMIF of every row whose "Advance Given?" is Yes),
// I2 remaining (=A2-E2) — all three read live so the site always shows
// exactly what the sheet shows, no re-derivation.
export async function readAdvanceBudget(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: `'${ADVANCE_TAB}'!A2:I2` });
  const r = res.data.values?.[0] || [];
  return { starting: parseNum(r[0]), committed: parseNum(r[4]), remaining: parseNum(r[8]) };
}

// ADVANCE CALCULATOR row = OUTREACH row + 2 (verified against the sheet's
// own row 7 formulas, which reference OUTREACH row 5) — every prospect row
// has a matching advance-calc row once the formula fill below covers it.
const outreachRowToAdvanceRow = (outreachRow) => outreachRow + 2;

// { [outreachRow]: "Yes" | "No" | "" } for every prospect row at once, so
// the outreach list can show each one's given status without a
// round trip per row.
export async function readAdvanceGivenMap(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: `'${ADVANCE_TAB}'!O7:O66` });
  const rows = res.data.values || [];
  const map = {};
  rows.forEach((r, i) => { map[OUTREACH_FIRST_ROW + i] = r[0] || ""; });
  return map;
}

export async function writeAdvanceGiven(sheets, spreadsheetId, outreachRow, given) {
  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `'${ADVANCE_TAB}'!O${outreachRowToAdvanceRow(outreachRow)}`,
    valueInputOption: "USER_ENTERED",
    requestBody: { values: [[given ? "Yes" : "No"]] },
  });
}

// One row per prospect: name, contact, the now-auto-summed Gross/Net
// Streams, status, and up to 5 song-level (streams, writer%) pairs so the
// site can show the same per-song breakdown staff see in the sheet. Stops
// at the "PIPELINE TOTAL" row, same convention as every other tab reader.
export async function readOutreachProspects(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: `'${OUTREACH_TAB}'!A${OUTREACH_FIRST_ROW}:AA64` });
  const rows = res.data.values || [];
  const totalIdx = rows.findIndex((r) => (r[0] || "").trim() === "PIPELINE TOTAL");
  const dataRows = totalIdx === -1 ? rows : rows.slice(0, totalIdx);
  return dataRows
    .map((r, i) => ({ r, row: OUTREACH_FIRST_ROW + i }))
    .filter(({ r }) => r[0])
    .map(({ r, row }) => ({
      row,
      name: r[0] || "",
      contact: r[1] || "",
      grossStreams: parseNum(r[2]),
      // both writerShare fields are read straight off percent-formatted
      // cells ("50.0%"), so parseNum leaves them on a 0-100 display scale —
      // fine for rendering as `${writerShare}%`, never used in the advance
      // math (which only ever needs netStreams).
      writerShare: parseNum(r[3]),
      netStreams: parseNum(r[4]),
      grossRevenue: parseNum(r[6]),
      adminFee: parseNum(r[7]),
      totalEstValue: parseNum(r[13]),
      status: r[14] || "",
      lastContact: r[15] || "",
      notes: r[16] || "",
      // all 5 slots, including empty ones (slot is 1-indexed to match the
      // write API) — the Outreach summary table only counts the filled
      // ones, but the edit UI needs every slot to know what's open.
      songs: [0, 1, 2, 3, 4].map((s) => ({
        slot: s + 1,
        streams: parseNum(r[17 + s * 2]),
        writerShare: parseNum(r[18 + s * 2]),
      })),
    }));
}

async function outreachTotalRowIndex(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: `'${OUTREACH_TAB}'!A${OUTREACH_FIRST_ROW}:A2000` });
  const rows = res.data.values || [];
  const idx = rows.findIndex((r) => (r[0] || "").trim() === "PIPELINE TOTAL");
  return { rows, totalRow: idx === -1 ? null : OUTREACH_FIRST_ROW + idx };
}

// Every row 5-64 already carries the pristine formula set from the 5-slot
// rework, so — unlike claimTabForNewCollaborator — adding a prospect never
// needs to write formulas, just find the first row whose name (col A) is
// still blank.
export async function firstEmptyOutreachRow(sheets, spreadsheetId) {
  const { rows, totalRow } = await outreachTotalRowIndex(sheets, spreadsheetId);
  const searchable = totalRow === null ? rows : rows.slice(0, totalRow - OUTREACH_FIRST_ROW);
  const idx = searchable.findIndex((r) => !r[0]);
  if (idx !== -1) return OUTREACH_FIRST_ROW + idx;
  if (totalRow !== null) {
    throw new Error(`No room left in the "${OUTREACH_TAB}" tab — every row above PIPELINE TOTAL is full.`);
  }
  return OUTREACH_FIRST_ROW + rows.length;
}

export async function writeOutreachProspect(sheets, spreadsheetId, row, { name, contact }) {
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId,
    requestBody: {
      valueInputOption: "USER_ENTERED",
      data: [
        { range: `'${OUTREACH_TAB}'!A${row}`, values: [[name]] },
        { range: `'${OUTREACH_TAB}'!B${row}`, values: [[contact || ""]] },
      ],
    },
  });
}

// Clears a prospect back to blank — name, contact, status/last-contact/
// notes, and all 5 song slots — but never the formula columns C:N (D/E/etc
// stay pristine, same as clearSongRow's rule for WRITER tabs) so the row
// is immediately ready for the next prospect.
export async function clearOutreachProspect(sheets, spreadsheetId, row) {
  await sheets.spreadsheets.values.batchClear({
    spreadsheetId,
    requestBody: {
      ranges: [
        `'${OUTREACH_TAB}'!A${row}`, `'${OUTREACH_TAB}'!B${row}`,
        `'${OUTREACH_TAB}'!O${row}`, `'${OUTREACH_TAB}'!P${row}`, `'${OUTREACH_TAB}'!Q${row}`,
        `'${OUTREACH_TAB}'!R${row}:AA${row}`,
      ],
    },
  });
}

const SONG_SLOT_COLS = [["R", "S"], ["T", "U"], ["V", "W"], ["X", "Y"], ["Z", "AA"]];

// writerShare arrives as a 0-100 number from the UI (matching how it
// reads back via parseNum on the percent-formatted cell) — divide by 100
// before writing so it lands on the same 0-1 fraction the cell's percent
// format expects.
export async function writeOutreachSong(sheets, spreadsheetId, row, slot, { streams, writerShare }) {
  const cols = SONG_SLOT_COLS[slot - 1];
  if (!cols) throw new Error("slot must be 1-5.");
  const [streamsCol, pctCol] = cols;
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId,
    requestBody: {
      valueInputOption: "USER_ENTERED",
      data: [
        { range: `'${OUTREACH_TAB}'!${streamsCol}${row}`, values: [[streams]] },
        { range: `'${OUTREACH_TAB}'!${pctCol}${row}`, values: [[writerShare / 100]] },
      ],
    },
  });
}

export async function clearOutreachSong(sheets, spreadsheetId, row, slot) {
  const cols = SONG_SLOT_COLS[slot - 1];
  if (!cols) throw new Error("slot must be 1-5.");
  await sheets.spreadsheets.values.batchClear({
    spreadsheetId,
    requestBody: { ranges: [`'${OUTREACH_TAB}'!${cols[0]}${row}`, `'${OUTREACH_TAB}'!${cols[1]}${row}`] },
  });
}

// One prospect, one save: name, contact, and up to 5 song slots all land
// in a single request instead of the caller having to fire off a separate
// write per field/song. `songs` is always all 5 slots (1-5); a slot with
// no streams and no writer share writes blank (clears it) rather than a
// literal 0, so an untouched or emptied-out slot looks pristine, not "0%".
export async function writeOutreachProspectFull(sheets, spreadsheetId, row, { name, contact, status, songs }) {
  const data = [
    { range: `'${OUTREACH_TAB}'!A${row}`, values: [[name]] },
    { range: `'${OUTREACH_TAB}'!B${row}`, values: [[contact || ""]] },
    { range: `'${OUTREACH_TAB}'!O${row}`, values: [[status || ""]] },
  ];
  for (const s of songs || []) {
    const cols = SONG_SLOT_COLS[s.slot - 1];
    if (!cols) continue;
    const streams = Number(s.streams) || 0;
    const writerShare = Number(s.writerShare) || 0;
    const hasData = streams > 0 || writerShare > 0;
    data.push({ range: `'${OUTREACH_TAB}'!${cols[0]}${row}`, values: [[hasData ? streams : ""]] });
    data.push({ range: `'${OUTREACH_TAB}'!${cols[1]}${row}`, values: [[hasData ? writerShare / 100 : ""]] });
  }
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId,
    requestBody: { valueInputOption: "USER_ENTERED", data },
...
// The exact values OUTREACH!O's dropdown (data validation) accepts —
// keep in sync with the sheet by hand if that list ever changes there.
const OUTREACH_STATUSES = ["Not Contacted", "Researching", "Contacted", "In Talks", "Offer Out", "Signed", "Passed"];

export { DATA_START_ROW, WRITER_TAB_RE, TEMPLATE_TAB_NAME, OUTREACH_TAB, ADVANCE_TAB, OUTREACH_STATUSES };
