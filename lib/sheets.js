...
export async function findUnclaimedRowByTitle(sheets, spreadsheetId, tabTitle, song) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const r = rows[i] || [];
    if (r[13]) continue; // already claimed by a Site Sync ID
...
export async function readMasterRollup(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "'MASTER'!A1:L300" });
  const rows = res.data.values || [];
  ...
  for (const r of dataRows) {
    ...
    if (!label) continue;
    if (label === "ROSTER TOTAL") { rosterTotal = toRow(r); break; }
    if (!parseNum(r[1])) continue; // empty placeholder WRITER slot — no songs logged
    writerRows.push(toRow(r));
  }
